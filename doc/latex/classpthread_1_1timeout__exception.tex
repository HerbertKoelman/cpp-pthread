\hypertarget{classpthread_1_1timeout__exception}{}\section{pthread\+:\+:timeout\+\_\+exception Class Reference}
\label{classpthread_1_1timeout__exception}\index{pthread\+::timeout\+\_\+exception@{pthread\+::timeout\+\_\+exception}}


{\ttfamily \#include $<$exceptions.\+hpp$>$}

Inheritance diagram for pthread\+:\+:timeout\+\_\+exception\+:\begin{figure}[H]
\begin{center}
\leavevmode
\includegraphics[height=3.000000cm]{classpthread_1_1timeout__exception}
\end{center}
\end{figure}
\subsection*{Public Member Functions}
\begin{DoxyCompactItemize}
\item 
\hyperlink{classpthread_1_1timeout__exception_aad3e5ac05faa1f0d8a7779678e70ad22}{timeout\+\_\+exception} (const std\+::string \&message)
\end{DoxyCompactItemize}


\subsection{Detailed Description}
pthread operation timed out. 

Definition at line 68 of file exceptions.\+hpp.



\subsection{Constructor \& Destructor Documentation}
\index{pthread\+::timeout\+\_\+exception@{pthread\+::timeout\+\_\+exception}!timeout\+\_\+exception@{timeout\+\_\+exception}}
\index{timeout\+\_\+exception@{timeout\+\_\+exception}!pthread\+::timeout\+\_\+exception@{pthread\+::timeout\+\_\+exception}}
\subsubsection[{\texorpdfstring{timeout\+\_\+exception(const std\+::string \&message)}{timeout\_exception(const std::string \&message)}}]{\setlength{\rightskip}{0pt plus 5cm}pthread\+::timeout\+\_\+exception\+::timeout\+\_\+exception (
\begin{DoxyParamCaption}
\item[{const std\+::string \&}]{message}
\end{DoxyParamCaption}
)\hspace{0.3cm}{\ttfamily [explicit]}}\hypertarget{classpthread_1_1timeout__exception_aad3e5ac05faa1f0d8a7779678e70ad22}{}\label{classpthread_1_1timeout__exception_aad3e5ac05faa1f0d8a7779678e70ad22}
thrown when a time out occurs.


\begin{DoxyParams}{Parameters}
{\em message} & timeout condition \\
\hline
\end{DoxyParams}


Definition at line 42 of file exceptions.\+cpp.



The documentation for this class was generated from the following files\+:\begin{DoxyCompactItemize}
\item 
include/pthread/exceptions.\+hpp\item 
src/exceptions.\+cpp\end{DoxyCompactItemize}
