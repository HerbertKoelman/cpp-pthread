...
namespace pthread {
    ...
    public:
         ...
         */
        virtual ~condition_variable() throw (condition_variable_exception);
    ...
    template<class Lambda>
    bool condition_variable::wait(mutex &mtx, Lambda lambda) {
    ...
    template<class Lambda>
    bool condition_variable::wait(lock_guard<pthread::mutex> &lck, Lambda lambda) {
    ...
    template<class Lambda>
    bool condition_variable::wait_for(mutex &mtx, int millis, Lambda lambda) {
        int rc = 0;
        ...
        while ((!stop_waiting) && (status == no_timeout)) {
            ...
            switch (rc) {
                ...
                case EINVAL:
                    throw condition_variable_exception("The value specified by abstime is invalid.",
                                                       rc); // NOSONAR we use throw instead of return.
                    break;

                case EPERM:
                    throw condition_variable_exception(
    ...
    template<class Lambda>
    bool condition_variable::wait_for(lock_guard<pthread::mutex> &lck, int millis, Lambda lambda) {
